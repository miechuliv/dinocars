...
class ModelCatalogCategory extends Model {
    ...
    public function getCategoryKeywords($id)
    {
        $result = $this->db->query("SELECT * FROM url_alias WHERE query = 'category_id=".$id."' ");
	...
	public function addCategory($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "',
		status = '" . (int)$data['status'] . "',
		virtual = '" . (int)$data['virtual'] . "',
        ...
        if ($data['keyword']) {
            if(is_array($data['keyword']))
            {
                foreach($data['keyword'] as $key => $keyword){
                    $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'category_id=" . (int)$category_id . "', keyword = '" . $this->db->escape($keyword) . "', `language`='".$key."' ");
                }

            }
            else
            {
                $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'category_id=" . (int)$category_id . "', keyword = '" . $this->db->escape($data['keyword']) . "', `language` = '".$this->config->get('config_language')."' ");
            }
	...
	public function editCategory($category_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "',
		 virtual = '" . (int)$data['virtual'] . "',
    ...
    public function getCategoriesExt($data,$lang_id) {
        $sql = "SELECT cp.category_id AS category_id, cd1.name AS name, c.parent_id, c.sort_order FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "category c ON (cp.path_id = c.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd1 ON (c.category_id = cd1.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd2 ON (cp.category_id = cd2.category_id)
        WHERE cd1.language_id = '".(int)$lang_id."'
...
public function getSiteMapCategories($parent_id = 0) {
    ...
    {
    $result =      $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category ptc LEFT JOIN category c ON(ptc.category_id=c.category_id) LEFT JOIN category_description cd ON(c.category_id = cd.category_id)  WHERE ptc.product_id = '" . (int)$product_id . "'");
