...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array(),$filtry = array()) {
      ...
      /*  if(isset($filters['cena_min']) OR isset($filters['cena_max'])){
              $sql .= "LEFT JOIN tax_class tc ON(p.tax_class_id=tc.tax_class_id) LEFT JOIN tax_rule tr ON(tc.tax_class_id=tr.tax_class_id) LEFT JOIN tax_rate trt ON(tr.tax_rate_id=trt.tax_rate_id) ";
        } */
        ...
        // miechu options
        if (isset($filtry['options']) AND $this->checkArrayEmpty($filtry['options'])) {
            foreach ($filtry['options'] as $key => $value) {
                if ($value!=="null" AND $value!=='')
                {

                    $sql.=" AND ovd.name='" . $value . "' AND od.name='" . $key . "' AND pov.quantity>=0 ";
        ...
        // miechu price filter
        if (isset($filtry['cena_min']) AND isset($filtry['cena_max'])) {
