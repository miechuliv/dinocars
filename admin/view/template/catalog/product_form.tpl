      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
          ...
          <div id="language<?php echo $language['language_id']; ?>">
            <table class="form">
              <tr>
                ...
                <td><input type="text" class="product-name" name="product_description[<?php echo $language['language_id']; ?>][name]" size="100" value="<?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['name'] : ''; ?>" />
          ...
          <table class="form">
            ...
            <script type="text/javascript">
                $('select[name="tax_class_id"]').change(function(){
                    $.ajax({
                        url: 'index.php?route=catalog/product/taxAjax&token=<?php echo $token; ?>&tax_class_id=' + encodeURIComponent($('select[name="tax_class_id"] option:selected').attr('value')) + '&price=' + $('input[name="price"]').attr('value'),
                        dataType: 'text',
                        success: function(text) {
                            $('input[name="price_brutto"]').val(text);
                        }
                    });
                })
            ...
            <tr>
              ...
              <tr>
                  ...
                  <td><select  name="feed" >
                          ...
                          <option value="1" <?php if($feed==1){ echo 'selected="selected"'; } ?> >Tak</option>
            ...
            <tr>
                ...
                <td>
                    <select name="google_merchant" >
                        <?php if ($google_merchant) { ?>
                        <option value="1" selected="selected"><?php echo $text_enabled; ?></option>
                        <option value="0"><?php echo $text_disabled; ?></option>
                        <?php } else { ?>
                        <option value="1"><?php echo $text_enabled; ?></option>
            ...
            <tr>
              ...
              <tr>
                  ...
                  <td><div class="scrollbox">
                      ...
                      <a onclick="$(this).parent().find(':checkbox').attr('checked', true);">Zaznacz wszystko</a> / <a onclick="$(this).parent().find(':checkbox').attr('checked', false);">Odznacz wszystko</a></td>
              </tr>
              <tr>
                  <td>Hurtownie</td>
                  <td><div class="scrollbox">
                          ...
                          <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
                          <div class="<?php echo $class; ?>">
                              <?php if (in_array($retailer['retailer_id'], $product_retailers)) { ?>
                              <input type="checkbox" name="product_retailer[]" value="<?php echo $retailer['retailer_id']; ?>" checked="checked" />
                              <?php echo $retailer['retailer_name']; ?>
                              <?php } else { ?>
                              <input type="checkbox" name="product_retailer[]" value="<?php echo $retailer['retailer_id']; ?>" />
                              <?php echo $retailer['retailer_name']; ?>
                              <?php } ?>
                          </div>
                          <?php } ?>
        ...
        <div id="tab-option">
          <div id="vtab-option" class="vtabs">
            ...
            <?php } ?>
            <span id="option-add">
            <select name="option"  style="width: 130px;" >
                <option></option>
...
function addAttribute() {
    ...
    html += '  <tr>';
	html += '    <td class="left"><select type="text" onchange="changeAttributeId(this)" name="product_attribute[' + attribute_row + ']"  ></select><input type="hidden" name="product_attribute[' + attribute_row + '][attribute_id]" /></td>';
	html += '    <td class="left">';
	<?php foreach ($languages as $language) { ?>
    ...
    function changeAttributeId(elem)
    {
        console.log($(elem).val());
        console.log($(elem).next());

        $(elem).next().val($(elem).find(':selected').val());
    }
...
function attributeautocomplete(attribute_row) {
   ...
   var  html = '<option></option>';
	...
	$('select[name=\'product_attribute[' + attribute_row + ']\']').append(html);
}

    $('#at_group').change(function(){

        var group_id = $('#at_group').find(":selected").val();


        loadAttrFromGroup(group_id);


    })

    function loadAttrFromGroup(group_id)
    {
        $.ajax({
            url: 'index.php?route=catalog/attribute/getbygroup&token=<?php echo $token; ?>&filter_attribute_group_id=' +  group_id,
            dataType: 'json',
            success: function(json) {

                $.each(json, function(index,value){
                    autoAttribute(value['attribute_id'],value['name'],value['values']);
                })
            }
        });
    }

    function autoAttribute(attr_id,name,value)
    {
        html  = '<tbody id="attribute-row' + attribute_row + '">';
        html += '  <tr>';
        html += '    <td class="left"><input type="text" name="product_attribute[' + attribute_row + '][name]" value="'+name+'" /><input type="hidden" name="product_attribute[' + attribute_row + '][attribute_id]" value="'+attr_id+'" /></td>';
        html += '    <td class="left">';
    <?php foreach ($languages as $language) { ?>
        html += '<textarea name="product_attribute[' + attribute_row + '][product_attribute_description][<?php echo $language['language_id']; ?>][text]" cols="40" rows="5"></textarea><img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" align="top" /><br />';
    <?php } ?>
...
var option_row = <?php echo $option_row; ?>;
    ...
    $('#option-add img').click(function(){


        if($('select[name=\'option\']').find(':selected').val() == '')
        {
            return false;
        }

        $.ajax({
            url: 'index.php?route=catalog/option/getoption&token=<?php echo $token; ?>&option_id=' +  encodeURIComponent($('select[name=\'option\']').find(':selected').val()),
            dataType: 'json',
            success: function(json) {
...
$('#vtab-option a').tabs();
    ...
    function addCode(elem,name)
    {
         var code = $(elem).parent().find('input').val();

         var html = '';

    html+= '<div>';
        html+= '<input type="hidden" name="'+name+'" value="'+code+'"  />'+code;
        html+= '<img src="view/image/delete.png" onclick="deleteCode(this)"  />';
        html+= '</div>';

         $(elem).parent().next('.scrollbox').append(html);
    }
